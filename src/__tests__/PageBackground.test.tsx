...
describe('PageBackground', () => {
  ...
  describe('mobile', () => {
    ...
    it('selects a background image from the m1-m18 pool', async () => {
      ...
      expect(src).toMatch(/^\/images\/m[1-9]\.png$/);
    });

    it('selects image m5 when Math.random returns 0.5', async () => {
      jest.spyOn(Math, 'random').mockReturnValue(0.5);
      mockHeaders.mockResolvedValue(makeHeaders('mobile'));
      const el = await PageBackground({ videoSrc: '/videos/test.mp4' });
      const { container } = render(el);
      // Math.floor(0.5 * 9) + 1 = 5
      expect(container.querySelector('img')).toHaveAttribute('src', '/images/m5.png');
    });
  });
